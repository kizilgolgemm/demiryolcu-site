...
const SALARY_FIELD_IDS=[
  'month','workMode','normalMesai','yillikIzin','sendikaTisIzin','ucretliIzin','ucretliRapor','haftaSonu','tatil','ubgtFiiliSaat','hatBakimSaat',
  ...
  'yurticiSefer','digerSgkDisi','sporAidati','lojman','prevGvMatrah','prevBonusReceived','prevBonusType','prevBonusGross','prevSgkMatrah','gvIstisna','dvIstisna',
...
function salaryPaidLeaveHours(){
  return round2(salaryAnnualLeaveHours() + salaryUnionLeaveHours());
}
...
function announcementImageSrc(item={}){
  return String(item.imageUrl || item.imageDataUrl || '').trim();
}
...
function bulletinFeatureMarkup(item){
  ...
  return `
    ...
    ${imageSrc ? `<img class="home-bulletin-image" src="${esc(imageSrc)}" alt="">` : ''}
...
function renderActiveProfileSummary(){
  ...
  if(el('profileDrawerAvatar')) el('profileDrawerAvatar').textContent=profileInitials(fullName, sicil);
...
function workTypeTemplate(type=sval('workType')){
  ...
  if(type==='ucretliIzin' || type==='yillikIzin' || type==='sendikaTisIzin' || type==='ucretliRapor') return {start:schedule.start,end:schedule.end,mealStart:schedule.mealStart,mealEnd:schedule.mealEnd};
...
function renderWorkEventList(summary){
  ...
  if(!summary.entries.length){
    wrap.innerHTML=summary.ucretliIzin || summary.ucretliRapor
      ? '<div class="leave-empty">Bu ay için çalışma kaydı yok; izin alanındaki yıllık ve Sendika TİS kayıtları maaş ekranında ayrı saatlere aktarıldı.</div>'
...
function renderWorkModule(){
  ...
  if(el('workCalendarHint')) el('workCalendarHint').textContent=summary.entries.length || summary.ucretliIzin ? 'Bu ayın çalışma takviminden fazla mesai, yıllık izin ve Sendika TİS izni maaş ekranına otomatik taşınır.' : 'Bu ay için kayıt yoksa fazla mesai ve izin alanları otomatik doldurulmaz.';
...
function fillCalendarInputs(resetBase=false){
  ...
  ['yillikIzin','sendikaTisIzin','ucretliIzin','ucretliRapor','tatil','ubgtFiiliSaat','hatBakimSaat','vardiyaSaat','geceSaat','kmSaat','rampaSefer','manevraSaat','beklemeGun','beklemeAdet'].forEach(id=>{ if(el(id)) el(id).value=0; });
  if(el('yillikIzin')) el('yillikIzin').value=workSummary.annualLeave || 0;
  if(el('sendikaTisIzin')) el('sendikaTisIzin').value=workSummary.unionLeave || 0;
  if(el('ucretliRapor')) el('ucretliRapor').value=workSummary.ucretliRapor || 0;
  syncPaidLeaveTotal();
...
function gatherResult(){
  ...
  const paidLeaveTotal=round2(annualLeaveHours + unionLeaveHours);
  const tayitHours=round2(nval('normalMesai') + unionLeaveHours + nval('haftaSonu') + nval('tatil') + nval('ubgtFiiliSaat'));
...
function renderOfficialPayroll(r){
  ...
  const normalTotal=round2(normalAmount+annualLeaveAmount+unionLeaveAmount+raporAmount+haftaSonuAmount+tatilAmount+ubgtFiiliAmount);
  const mode=sval('workMode');
  const vardiyaSaat=isWorkModeAllowed('vardiyaSaat', mode) ? nval('vardiyaSaat') : 0;
  const geceSaat=isWorkModeAllowed('geceSaat', mode) ? nval('geceSaat') : 0;
  const hatBakimSaat=isWorkModeAllowed('hatBakimSaat', mode) ? nval('hatBakimSaat') : 0;
...
function loadFirebaseForumTopics(force=false){
  ...
  if(!bridge || typeof bridge.loadForumTopics!=='function'){
    setForumStatus('Forum bağlantısı için Firebase web köprüsü gerekli. Sitede firebase-web.js dosyasının yüklendiğinden emin ol.', 'warn');
    return false;
  }
  if(firebaseForumLoading || (firebaseForumLoaded && !force)) return true;
  firebaseForumLoading=true;
  setForumStatus('Forum konuları Firebase üzerinden alınıyor...');
  try{
...
function submitFirebaseForumTopic(){
  ...
  if(!bridge || typeof bridge.createForumTopic!=='function'){
    setForumStatus('Forum şu anda Firebase bağlantısı bulamadı. Dosyaları siteye yükledikten sonra tekrar dene.', 'warn');
    return false;
  }
  ...
  try{
    ...
    setForumStatus('Konu Firebase forum havuzuna gönderiliyor...');
    return true;
  }catch(error){
...
function submitFirebaseForumReply(){
  ...
  if(!bridge || typeof bridge.createForumReply!=='function'){
    setForumStatus('Forum cevap bağlantısı için Firebase web köprüsü gerekli.', 'warn');
    return false;
  }
...
function renderForum(){
  ...
  if(!session?.active || isGuestSession(session)){
    ...
    setForumStatus(`${currentForumNick()} nickiyle forumdasın. Ad, soyad ve sicil forumda gösterilmez.`);
  }
...
function renderForumTopicList(){
  ...
  if(!node) return;
  const topics=activeForumTopics();
  if(el('forumTopicCount')) el('forumTopicCount').textContent=`${topics.length} konu`;
  if(firebaseForumLoading){
    node.innerHTML='<div class="forum-empty">Konular yükleniyor...</div>';
    return;
  }
  if(!topics.length){
    node.innerHTML='<div class="forum-empty">Henüz konu yok. İlk başlığı açarak forumu başlatabilirsin.</div>';
    return;
  }
  ...
  node.innerHTML=topics.map(item=>{
    ...
    return `<button type="button" class="forum-topic-row${active}" data-forum-topic="${esc(item.topicId)}">
      <div class="forum-topic-meta"><span>${esc(forumCategoryLabel(item.category))}</span><span>${esc(forumDateLabel(item.lastActivityAt || item.createdAt))}</span><span>${item.replyCount || 0} cevap</span></div>
      <h4>${esc(item.title)}</h4>
      <p>${esc(forumExcerpt(item.body))}</p>
      <div class="forum-topic-meta"><span>${esc(item.authorName)}</span></div>
    </button>`;
  }).join('');
}
function renderForumTopicDetail(){
  ...
  const topic=selectedForumTopic();
  if(!topic){
    node.innerHTML='<div class="forum-empty">Bir konu seçildiğinde detay ve cevaplar burada görünür.</div>';
    return;
  }
  ...
  const replyMarkup=firebaseForumRepliesLoading
    ...
    : replies.length
      ...
      : '<div class="forum-empty">Bu konuya henüz cevap yazılmadı.</div>';
  node.innerHTML=`<div class="forum-detail-title">
    <div class="forum-detail-meta"><span>${esc(forumCategoryLabel(topic.category))}</span><span>${esc(forumDateLabel(topic.createdAt))}</span><span>${topic.replyCount || 0} cevap</span></div>
    ...
    ? list.slice(0,20).map(item=>{
      ...
      return `<div class="preview-item notification-history-item">
        <div class="notification-history-main">
          <div class="label">${esc(item.source || 'Duyuru')} • ${esc(item.label)} • ${esc(item.target || 'app_platform')} • ${esc(item.audience || 'installed_members')} • ${esc(item.channel || 'app')} • ${esc(item.createdAt ? new Date(item.createdAt).toLocaleString('tr-TR') : item.date || '-')}${item.installTokenCount ? ' • '+esc(item.installTokenCount)+' cihaz' : ''}</div>
...
function renderNotificationImagePreview(){
  ...
  node.innerHTML=src
    ? `<img src="${esc(src)}" alt=""><div><strong>Seçili görsel</strong><span>Bu görsel Güncel Duyurular ve uygulama karşılama kartında kullanılır.</span></div>`
    : '';
}
function handleNotificationImageFile(){
  ...
  if(file.size>700*1024){
    if(el('notificationStatus')) el('notificationStatus').textContent='Görsel çok büyük. Firebase için 700 KB altında bir görsel seç veya Görsel URL kullan.';
    input.value='';
    renderNotificationImagePreview();
    return;
  }
  ...
  reader.onload=()=>{
    notificationSelectedImageDataUrl=String(reader.result || '');
    renderNotificationImagePreview();
  };
  reader.onerror=()=>{
    notificationSelectedImageDataUrl='';
...
window.onFirebaseBridgeResult=function(action,success,payloadJson,message){
  ...
  if(action==='loadForumTopics'){
    firebaseForumLoading=false;
    firebaseForumLoaded=true;
    firebaseForumTopics=success && Array.isArray(payload)
      ? payload.map(normalizeForumTopic)
      : [];
  ...
  if(action==='loadForumReplies'){
    firebaseForumRepliesLoading=false;
    firebaseForumReplies=success && Array.isArray(payload)
      ? payload.map(normalizeForumReply)
      : [];
    renderForum();
    return;
  }
  ...
  if(action==='loadAnnouncements'){
    firebaseAnnouncementsLoading=false;
    firebaseAnnouncements=success && Array.isArray(payload)
      ? payload.map(normalizeAnnouncement).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0))
      : [];
    renderHomeBulletins();
    renderNotificationHistory();
  ...
  if(action==='createAnnouncement'){
    if(success){
      ...
      clearNotificationInputs();
      if(el('notificationStatus')) el('notificationStatus').textContent=message || 'Duyuru yayınlandı.';
      const whatsappButton=el('openWhatsappShareBtn');
      if(whatsappButton) whatsappButton.classList.toggle('hidden', !notificationLastWhatsappUrl);
      if(notificationLastWhatsappUrl) openWhatsappShare(notificationLastWhatsappUrl);
  ...
  el('membershipAdminPreview').innerHTML=`<div class="membership-admin-detail">
    <div class="membership-admin-section membership-review-card">
      <div class="membership-review-profile">
        ...
        <div class="membership-review-avatar">${esc(identityInitials)}</div>
